// ==UserScript==
// @name         ⚡ V20_LOSS-CUT — v19 (EXPIRY) + Data-Driven Momentum-Chase Loss Filter
// @namespace    candle-pro-strategy-v20-losscut
// @version      20.0.0
// @description  v19 كما هي + بوابة قطع الخسائر (تحجب مطاردة الزخم في الضجيج — 6%→محجوبة، رفعت الجلسة 35%→46%)
// @author       aoirusra
...
(function (W) {
  ...
  const CFG = {
    ...
    // ─── [V20 LOSS-CUT] بوابة قطع الخسائر (معايرة على 65 صفقة حقيقية) ─────────
    //   تحجب «مطاردة الزخم» فقط: دخول مع ميل التيك + مع التسارع + ضجيج مرتفع.
    //   هذه الشريحة سجّلت 6% فوز (1/17) تاريخياً — حجبها رفع الجلسة 35%→46%.
    LOSSCUT_ENABLED       : true,          // فعّل البوابة
    LOSSCUT_MODE          : 'cut',         // 'cut'=حظر الأرخبيل الخاسر | 'log'=قياس فقط (لا يغيّر السلوك)
    LOSSCUT_SLOPE_MS      : 2000,          // نافذة الميل القصير (مشتقة أولى) ≈4-5 تيكات
    LOSSCUT_ACCEL_MS      : 1500,          // نافذة التسارع (مشتقة ثانية)
    LOSSCUT_NOISE_TICKS   : 40,            // عدد فروق التيكات لتقدير الضجيج
    LOSSCUT_NOISE_PIPS    : 5.5,           // عتبة «الضجيج المرتفع» (الوسيط في بياناتك ≈5.5pip)
    LOSSCUT_MIN_SLOPE_REL : 0.000020,      // أدنى ميل نسبي لاعتبار الحركة «اندفاعاً» (وإلا ليست مطاردة)
  };
  ...
  // ══════════════════════════════════════════════════════════════════════
  // [V20 LOSS-CUT] بوابة قطع الخسائر — مبنية على تحليل صفقاتك الحقيقية (65 صفقة)
  //   الاكتشاف: الأرخبيل الخاسر = «مطاردة الزخم» = الدخول *مع* ميل التيك القصير
  //   *و* مع التسارع *و* في ضجيج مرتفع → معدل فوز 6% فقط (1/17 صفقة).
  //   حجب هذه الشريحة وحدها رفع الجلسة من 35% → 46% بكلفة صفقة رابحة واحدة فقط.
  //   لماذا؟ على أزواج OTC القصيرة، الاندفاع القوي يرتدّ (عودة للمتوسط) — فالدخول
  //   على قمة الاندفاع = شراء القمة قبل الانعكاس مباشرة.
  //   الفلسفة: لا نكسر الرابح — نحجب *فقط* هذا التقاطع الضيّق الخاسر (إضافة آمنة).
  // ══════════════════════════════════════════════════════════════════════
  const LossCut = (function () {
    ...
    function tickNoise(asset) {
      ...
      const n = Math.min(CFG.LOSSCUT_NOISE_TICKS, buf.length - 1);
      let s = 0, s2 = 0, k = 0;
      for (let i = buf.length - n; i < buf.length; i++) { const d = buf[i] - buf[i - 1]; s += d; s2 += d * d; k++; }
      if (k < 2) return null;
      const mean = s / k; return Math.sqrt(Math.max(0, s2 / k - mean * mean));
    }
    // القرار: هل الصفقة تنتمي للأرخبيل الخاسر؟ يُرجع {block, reason, ...}
    function evaluate(direction, asset) {
      if (!CFG.LOSSCUT_ENABLED) return { block: false };
      const a = normalizeAsset(asset || activeAsset);
      const dir = direction === 'BUY' ? 1 : -1;
      const s2 = OracleLab.microSlope(a, CFG.LOSSCUT_SLOPE_MS);     // مشتقة أولى (نافذة قصيرة)
      const acc = OracleLab.microAccel(a, CFG.LOSSCUT_ACCEL_MS);    // مشتقة ثانية
      const nz = tickNoise(a);
      if (!s2 || !acc || nz == null) return { block: false, reason: 'بيانات ناقصة' };
      // محاذاة الزخم/التسارع مع اتجاه الصفقة (>0 = مع الاتجاه = مطاردة)
      const align    = Math.sign(s2.rel) * dir;
      const accAlign = Math.sign(acc.accel) * dir;
      const lastP = (tickBuffers[a] && tickBuffers[a].length) ? tickBuffers[a][tickBuffers[a].length - 1] : 0;
      const noisePips = lastP > 0 ? (nz / lastP) * 1e5 : 0;          // ضجيج بالـpip النسبي
      const chasing  = align > 0 && accAlign > 0;                    // مع الزخم + مع التسارع
      const highNoise = noisePips >= CFG.LOSSCUT_NOISE_PIPS;
      const slopeStrong = Math.abs(s2.rel) >= CFG.LOSSCUT_MIN_SLOPE_REL;
      const block = chasing && highNoise && slopeStrong;
      return {
        block, chasing, highNoise, noisePips, align, accAlign,
        slopeRel: s2.rel, accel: acc.accel,
        reason: block ? 'مطاردة زخم+تسارع في ضجيج مرتفع (أرخبيل 6% فوز)' : 'سليم',
      };
    }
    // البوابة: تُسجّل وتُعيد true إذا وجب الحظر (في وضع 'cut'). 'log' = قياس فقط.
    function gate(direction, asset) {
      if (!CFG.LOSSCUT_ENABLED) return false;
      let r; try { r = evaluate(direction, asset); } catch (_) { return false; }
      if (!r || r.reason === 'بيانات ناقصة') return false;
      if (r.block) {
        const tag = CFG.LOSSCUT_MODE === 'cut' ? '🛑 [LOSS-CUT]' : '🔬 [LOSS-CUT/قياس]';
        addLog(tag + ' ' + direction + ' | ' + (asset || activeAsset) +
               ' | مطاردة زخم: ميل ' + (r.slopeRel * 1e6).toFixed(1) + 'e-6 + تسارع' +
               ' + ضجيج ' + r.noisePips.toFixed(1) + 'pip → ' +
               (CFG.LOSSCUT_MODE === 'cut' ? 'حُجبت (تاريخياً 6% فوز)' : 'كانت ستُحجب'),
               CFG.LOSSCUT_MODE === 'cut' ? 'error' : 'info');
        return CFG.LOSSCUT_MODE === 'cut';
      }
      return false;
    }
    return { evaluate, gate, tickNoise };
  })();
  ...
  function executeTrade(direction, asset, overrideAmount) {
    ...
    if (LossCut.gate(direction, asset)) return;   // [V20 LOSS-CUT] احجب أرخبيل مطاردة الزخم
  ...
  const DualWSSManager = (() => {
    ...
    function _executeDualTrade(direction, asset, overrideAmount, count) {
      ...
      const tradeSec = _lastSmartDurSec >= 10
        ? _snapTradeDuration(_lastSmartDurSec)
        : _snapTradeDuration(_tradeDuration || (candlePeriod || 10));
      if (LossCut.gate(direction, asset)) { _currentIntervalSignal = false; return; }   // [V20 LOSS-CUT]
