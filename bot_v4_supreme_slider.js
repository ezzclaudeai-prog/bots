...
(function (W) {
  ...
  const CFG = {
    ...
    // ─── [V26] TickForecast — محرك الإسقاط الاحتمالي الأمامي (عملية حسابية دقيقة) ──
    //   يحسب من التيكات الخام: متى يأتي التيك القادم (cadence)، الانجراف لكل تيك،
    //   التذبذب، ثم يُسقط السعر للأمام N تيك ويحسب احتمال الفوز pWin والقيمة EV
    //   وأفضل مدة. بوابة EV تمنع الدخول السالب القيمة، ومراقب الصحة يعيد الحساب أثناء الصفقة.
    FC_ENABLED              : true,        // ✅ تفعيل الإسقاط الأمامي (pWin/EV/أفضل مدة)
    FC_GATE_ENABLED         : true,        // ✅ بوابة EV: امنع الدخول الخاسر إحصائياً
    FC_GATE_BLOCK           : true,        // true=يحجب فعلياً | false=استشاري فقط (يسجّل بلا منع)
    FC_DURATION_ENABLED     : true,        // ✅ استخدم أفضل مدة محسوبة بدل الثابتة
    FC_HEALTH_ENABLED       : true,        // ✅ مراقب صحة الصفقة المفتوحة (تحذير اخرج/تمسّك)
    FC_WINDOW_MS            : 12000,       // نافذة التيكات الخام للإحصاء (مللي)
  ...
  function _snapTradeDuration(secs) {
    // ✅ [INTERVAL] الحد الأدنى 10 ثواني — لا صفقات 3,5,7 ثواني
    if (CFG.SCALP_FIXED_ENABLED) {
      let fx = CFG.SCALP_FIXED_SEC || 10;
      if (fx < (CFG.MIN_TRADE_SEC || 10)) fx = (CFG.MIN_TRADE_SEC || 10);
      if (fx > (CFG.SCALP_MAX_SEC || 60)) fx = (CFG.SCALP_MAX_SEC || 60);
      return snapToPOTime(fx);
    }
    const s = Math.round(Number(secs) || 0);
    return Math.max(CFG.MIN_TRADE_SEC || 10, s); // ✅ [INTERVAL] حد أدنى 10 ثواني
  }
  ...
  const OracleLab = (function () {
    ...
    return { onTick, microSlope, microAccel, lastSpike, snapshot, recordClose, report, _stats: stats };
  })();
  try { W._oracleLabReport = () => OracleLab.report(); } catch (_) {}

  // ═══════════════════════════════════════════════════════════════════════
  // § 3.b  [V26] TickForecast — محرك الإسقاط الاحتمالي الأمامي (Forward Tick Engine)
  //   الفكرة (طلب المستخدم): بدل «اتجاه عشوائي» أو زخم خام، نعقد عملية حسابية
  //   دقيقة على التيكات الخام لنُسقط السعر للأمام N تيك ونحسب:
  //     • cadence  : وسيط الفاصل بين التيكات → متى يصل التيك القادم (≈470-500ms للـOTC)
  //     • drift    : ميل عائد لوغاريتمي مُشذّب (مقاوم للقفزات) لكل تيك
  //     • sigma    : تذبذب العائد لكل تيك
  //     • pWin     : احتمال الفوز = CDF طبيعي لمشي عشوائي بانجراف على الأفق
  //     • EV       : pWin*payout − (1−pWin)
  //     • bestHorizon : المدة التي تُعظّم pWin بين مدد المنصة المتاحة
  //   نموذج: العائد اللوغاريتمي التراكمي على N تيك ~ Normal(N·drift, (sigma·√N)²).
  //   ملاحظة صادقة: المنصّة لا تتيح إغلاقاً مبكراً للخيارات الثنائية، لذا «تجنّب
  //   الخسارة» يتحقق عبر اختيار الدخول والمدة (بوابة EV) لا عبر خروج لحظي. مراقب
  //   الصحة يُعيد حساب pWin أثناء الصفقة كمؤشّر استشاري (HUD) وكبيانات تعلّم.
  // ═══════════════════════════════════════════════════════════════════════
  const TickForecast = (function () {
    const buf = {};            // asset -> [{t,p}]
    const MAXLEN = 400;
    function onTick(a, price, t) {
      if (!(price > 0) || !(t > 0)) return;
      const b = buf[a] || (buf[a] = []);
      b.push({ t, p: price });
      const cut = t - (CFG.FC_WINDOW_MS || 12000);
      while (b.length && b[0].t < cut) b.shift();
      if (b.length > MAXLEN) b.shift();
    }
    // CDF طبيعي قياسي عبر تقريب erf (Abramowitz-Stegun 7.1.26، دقّة ~1.5e-7)
    function _erf(x) {
    ...
    function _median(arr) {
      ...
      const c = arr.slice().sort((x, y) => x - y), m = c.length >> 1;
      return c.length % 2 ? c[m] : (c[m - 1] + c[m]) / 2;
    }
    // إحصاءات التيكات الخام على النافذة: cadence / drift / sigma لكل تيك
    function stats(a) {
      const b = buf[a];
      const minT = (CFG.FC_MIN_TICKS || 6);
      if (!b || b.length < minT) return null;
      const rets = [], dts = [];
      for (let i = 1; i < b.length; i++) {
        const p0 = b[i - 1].p, p1 = b[i].p, dt = b[i].t - b[i - 1].t;
        if (p0 > 0 && p1 > 0 && dt > 0 && dt < 6000) { rets.push(Math.log(p1 / p0)); dts.push(dt); }
      }
      if (rets.length < minT - 1) return null;
      const cadence = _median(dts) || 500;
      const driftMed = _median(rets);                       // اتجاه مقاوم للقفزات
      let sum = 0; for (const r of rets) sum += r;
      const driftMean = sum / rets.length;                  // حجم
      let v = 0; for (const r of rets) v += (r - driftMean) * (r - driftMean);
      const sigma = Math.sqrt(v / Math.max(1, rets.length - 1)) || 1e-7;
      // انجراف فعّال متحفّظ: نأخذ الأصغر حجماً بين الوسيط والمتوسط بنفس الإشارة؛
      //   تعارض إشارة الوسيط والمتوسط = لا انجراف موثوق (نُصفّره).
      let drift;
    ...
    //   بمستوى الضجيج «حافة مؤكّدة» فيبالغ في الثقة على سوق هادئ (مشكلة V13 العشوائية).
    //   Var = σ²·N + N²·(σ²/n) ⇒ sd = σ·√(N + N²/n). يجعل pWin محدوداً بدلالة معنوية
    //   الانجراف الإحصائية (t‑stat)، فلا يصل أبداً إلى 100% من ضجيج.
    function _predSd(sigma, N, n) {
      const nS = Math.max(2, n || 2);
      return sigma * Math.sqrt(N + (N * N) / nS);
    }
    // إسقاط أمامي على أفق H مللي: السعر المتوقع وحزمة الثقة ±2σ تنبّؤي
    function project(a, horizonMs) {
      const s = stats(a); if (!s) return null;
      const N = Math.max(1, horizonMs / s.cadence);
      const mu = s.drift * N;                       // عائد لوغاريتمي متوقع تراكمي
      const sd = _predSd(s.sigma, N, s.n);          // انحراف تنبّؤي (انتشار + عدم يقين الانجراف)
      return {
        priceHat: s.lastPrice * Math.exp(mu),
        lo: s.lastPrice * Math.exp(mu - 2 * sd),
        hi: s.lastPrice * Math.exp(mu + 2 * sd),
        nTicks: N, mu, sd, sigma: s.sigma, drift: s.drift, cadence: s.cadence, nSample: s.n, lastPrice: s.lastPrice,
      };
    }
    // احتمال فوز خيار ثنائي يُفتح عند السعر الحالي (strike=السعر) باتجاه dir على أفق H.
    //   شرط الفوز CALL: السعر النهائي > سعر الفتح (+هامش انزلاق). PUT: أقل.
    function winProb(a, dir, horizonMs) {
      const pr = project(a, horizonMs); if (!pr || pr.sd <= 0) return null;
      const buffer = (CFG.FC_EDGE_BUFFER_REL || 0.000005);   // ≈ سبريد نصفي/انزلاق (عائد لوغاريتمي)
      const z = dir === 'BUY'
        ? (pr.mu - buffer) / pr.sd
        : (-pr.mu - buffer) / pr.sd;
      return { pWin: _normCdf(z), z, ...pr };
    }
    function ev(pWin, payout) { const rho = (payout != null ? payout : (CFG.FC_PAYOUT || 0.92)); return pWin * rho - (1 - pWin); }
    // أفضل مدة بين مدد المنصة المتاحة: تُعظّم pWin (الأفق الأطول يرفع z عند وجود انجراف)
    function bestHorizon(a, dir, candidatesSec) {
      const cands = (candidatesSec && candidatesSec.length) ? candidatesSec : (CFG.FC_HORIZONS || [10, 15, 20, 30, 45, 60]);
      let best = null;
      for (const sec of cands) {
        const w = winProb(a, dir, sec * 1000);
        if (!w) continue;
        if (!best || w.pWin > best.pWin) best = { sec, pWin: w.pWin, ev: ev(w.pWin), z: w.z };
      }
      return best;
    }
    // قرار الدخول: الاتجاه المُفضّل + أفضل مدة + pWin/EV + هل يُسمح
    function decide(a, dirHint) {
      const s = stats(a); if (!s) return { ok: false, reason: 'no_ticks' };
      const driftDir = s.drift > 0 ? 'BUY' : (s.drift < 0 ? 'SELL' : null);
      const dir = dirHint || driftDir;
      if (!dir) return { ok: false, reason: 'flat_drift', drift: s.drift, sigma: s.sigma, cadence: s.cadence };
      const bh = bestHorizon(a, dir);
    ...
    //   على الزمن المتبقّي. مؤشّر استشاري — المنصّة لا تتيح إغلاقاً مبكراً للثنائيات.
    function health(a, dir, openPrice, elapsedMs, totalMs) {
      const s = stats(a); if (!s || !(openPrice > 0)) return null;
      const remMs = Math.max(0, totalMs - elapsedMs);
      const cushion = Math.log(s.lastPrice / openPrice) * (dir === 'BUY' ? 1 : -1);  // ربح حالي لوغاريتمي
      if (remMs <= 0 || s.sigma <= 0) return { pWin: cushion > 0 ? 1 : 0, cushion, remMs, itm: cushion > 0, nTicks: 0 };
      const N = Math.max(1, remMs / s.cadence);
      const mu = (dir === 'BUY' ? 1 : -1) * s.drift * N;
      const sd = _predSd(s.sigma, N, s.n);    // انحراف تنبّؤي (يشمل عدم يقين الانجراف)
  ...
  function _rebuildPayloadCache() {
    ...
    const dur = _lastSmartDurSec >= 10 ? _lastSmartDurSec : (_tradeDuration > 0 ? _tradeDuration : (candlePeriod || 10));
  ...
  function processHistoryFast(asset, period, history) {
    ...
    // [V26] إصلاح كشف الفريم الديناميكي: لا نلتقط الفريم مرة واحدة فقط —
    //   نقبل تغيّره عندما يرسل الموقع فريماً مختلفاً (بعد انتهاء قفل 30ث)،
    //   ونمسح شموع الزوج كي لا تختلط شموع فريمين (5ث مع 15ث…).
    if (period && period > 0) {
      if (!candlePeriod) { candlePeriod = period; durSource = 'history'; updateHUD(); }
      else if (period !== candlePeriod && Date.now() >= (_periodLockUntil || 0)) {
        addLog('🕒 [FRAME] تغيّر الفريم ' + candlePeriod + 'ث → ' + period + 'ث (history) — إعادة بناء الشموع', 'info');
        try { candleBuffers[a] = []; currentCandles[a] = null; } catch(_) {}
        onPlatformTimeframe(period, 'history');
      }
    }
  ...
  function onOpenOrderSuccess(data) {
    ...
    try {
      const _closeEl = W.document.getElementById('cbSpCloseTime');
      const _countEl = W.document.getElementById('cbSpCountdown');
      const _durEl   = W.document.getElementById('cbSpDuration');
      const _priceEl = W.document.getElementById('cbSpPrice');
      if (_closeEl) _closeEl.textContent = closeHMS;
      if (_durEl)   _durEl.textContent = durSec + ' ثانية';
  ...
  function onTick(asset, price, serverTs, srcRole) {
    ...
    try { if (CFG.FC_ENABLED) TickForecast.onTick(a, price, labTs); } catch(_) {}   // [V26] إطعام محرك الإسقاط الأمامي
    try {
      // [V26] مراقب صحة الصفقة المفتوحة — عملية حسابية مستمرة على التيكات (استشاري)
      if (CFG.FC_ENABLED && CFG.FC_HEALTH_ENABLED && tradeExec && _pendingTradeRecord && _pendingTradeRecord.openPrice > 0
          && normalizeAsset(_pendingTradeRecord.asset || a) === a) {
        const _ptr = _pendingTradeRecord;
        const _totSec = (_lastSmartDurSec >= 10 ? _lastSmartDurSec : (_tradeDuration || candlePeriod || 10));
        const _elapsed = Date.now() - (_ptr.openTs || Date.now());
        const _h = TickForecast.health(a, _ptr.direction, _ptr.openPrice, _elapsed, _totSec * 1000);
        if (_h) {
          _fcUpdateHealthHUD(_h);
          const _nowH = Date.now();
          if (_h.pWin < (CFG.FC_HEALTH_DANGER || 0.40) && _nowH - _fcLastHealthLog > 1500) {
            _fcLastHealthLog = _nowH;
            addLog('⚠️ [FORECAST-HEALTH] ' + _ptr.direction + ' بخطر — pWin ' + Math.round(_h.pWin * 100) + '% | ' +
                   (_h.itm ? 'رابح الآن' : 'خاسر الآن') + ' | متبقٍّ ' + (Math.round(_h.remMs / 100) / 10) + 'ث', 'info');
          }
        }
      }
    } catch(_) {}
  ...
  /* ══ SUPREME-PRED V2 V13 — إشعار إشارة بنمط «احسنت» (وسط الشاشة) ══ */
  #cbSignalPopup{position:fixed;top:8%;left:6%;right:6%;transform:translateY(-30px) scale(0.95);z-index:2147483647;width:auto;max-width:calc(100vw - 12%);background:rgba(30,34,42,0.94);border:none;border-radius:10px;padding:0;overflow:hidden;opacity:0;pointer-events:none;transition:all 0.35s cubic-bezier(0.34,1.56,0.64,1);direction:rtl;font-family:'IBM Plex Sans Arabic',-apple-system,BlinkMacSystemFont,sans-serif;box-shadow:0 8px 32px rgba(0,0,0,0.6);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);}
  #cbSignalPopup.visible{opacity:1;transform:translateY(0) scale(1);pointer-events:auto;}
  #cbSignalPopup.buy-signal{box-shadow:0 8px 32px rgba(0,0,0,0.6),0 0 24px rgba(0,210,100,0.15);}
  #cbSignalPopup.sell-signal{box-shadow:0 8px 32px rgba(0,0,0,0.6),0 0 24px rgba(220,38,38,0.15);}
  .sp-header{padding:10px 14px 8px;text-align:center;position:relative;}
  .sp-header.buy-bg{background:linear-gradient(135deg,rgba(15,42,28,0.7),rgba(30,58,47,0.7));}
  .sp-header.sell-bg{background:linear-gradient(135deg,rgba(42,20,22,0.7),rgba(58,26,26,0.7));}
  .sp-dir-icon{font-size:24px;line-height:1;margin-bottom:2px;}
  .sp-dir-text{font-size:18px;font-weight:900;letter-spacing:0.5px;}
  .sp-dir-text.buy-text{color:#46d98e;}
  .sp-dir-text.sell-text{color:#DC2626;}
  .sp-pattern{font-size:9px;color:#9fb2c0;margin-top:2px;font-weight:500;}
  .sp-body{padding:8px 14px 10px;display:flex;flex-wrap:wrap;gap:6px;}
  .sp-row{display:flex;align-items:center;justify-content:space-between;padding:5px 10px;background:rgba(22,34,46,0.6);border:none;border-radius:6px;flex:1 1 calc(50% - 3px);min-width:0;}
  .sp-row-label{font-size:10px;color:#9fb2c0;font-weight:600;display:flex;align-items:center;gap:4px;white-space:nowrap;}
  .sp-row-label .sp-icon{font-size:12px;}
  .sp-row-value{font-size:13px;font-weight:800;font-family:'SF Mono',ui-monospace,monospace;color:#eef3f7;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
  .sp-row-value.green{color:#46d98e;}
  .sp-row-value.red{color:#DC2626;}
  .sp-row-value.yellow{color:#D97706;}
  .sp-row-value.cyan{color:#3fe0ff;}
  .sp-countdown-bar{margin-top:2px;height:4px;border-radius:2px;background:rgba(36,52,67,0.5);overflow:hidden;flex:1 1 100%;}
  .sp-countdown-fill{height:100%;border-radius:2px;background:linear-gradient(90deg,#00d264,#3fe0ff);transition:width 1s linear;}
  .sp-countdown-fill.sell-fill{background:linear-gradient(90deg,#DC2626,#ff6b6b);}
  .sp-conf-row{display:flex;align-items:center;gap:6px;padding:5px 10px;background:rgba(22,34,46,0.6);border:none;border-radius:6px;flex:1 1 calc(50% - 3px);min-width:0;}
  .sp-conf-label{font-size:10px;color:#9fb2c0;font-weight:600;white-space:nowrap;}
  .sp-conf-track{flex:1;height:5px;border-radius:3px;background:rgba(36,52,67,0.5);overflow:hidden;}
  .sp-conf-fill{height:100%;border-radius:3px;transition:width 0.5s;}
  .sp-conf-fill.high{background:linear-gradient(90deg,#16A34A,#46d98e);}
  .sp-conf-fill.mid{background:linear-gradient(90deg,#D97706,#FCD34D);}
  .sp-conf-fill.low{background:linear-gradient(90deg,#DC2626,#FCA5A5);}
  .sp-conf-val{font-size:13px;font-weight:900;font-family:'SF Mono',ui-monospace,monospace;color:#eef3f7;min-width:30px;text-align:right;}
  .sp-footer{padding:6px 14px 10px;text-align:center;}
  .sp-close-btn{padding:6px 28px;border-radius:20px;border:1px solid rgba(36,52,67,0.6);background:rgba(22,34,46,0.6);color:#9fb2c0;font-family:inherit;font-size:10px;font-weight:700;cursor:pointer;transition:all 0.2s;}
  .sp-close-btn:hover{background:rgba(15,42,28,0.8);border-color:#46d98e;color:#46d98e;}
  .sp-overlay{display:none;}
  .sp-title-bar{display:flex;align-items:center;justify-content:center;gap:5px;padding:6px 12px;background:linear-gradient(90deg,rgba(15,42,28,0.8) 0%,rgba(30,58,47,0.8) 50%,rgba(15,42,28,0.8) 100%);border-bottom:1px solid rgba(36,52,67,0.4);font-size:10px;font-weight:900;color:#46d98e;letter-spacing:1.5px;text-transform:uppercase;}
  .sp-title-bar .sp-title-icon{font-size:11px;}
  .sp-timer-badge{display:inline-block;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:900;font-family:'SF Mono',ui-monospace,monospace;letter-spacing:0.5px;}
  .sp-timer-badge.buy{background:rgba(70,217,142,0.15);color:#46d98e;border:1px solid rgba(70,217,142,0.3);}
  .sp-timer-badge.sell{background:rgba(220,38,38,0.15);color:#DC2626;border:1px solid rgba(220,38,38,0.3);}
  @keyframes spSlideIn{from{transform:translateY(-30px) scale(0.95);opacity:0;}to{transform:translateY(0) scale(1);opacity:1;}}
  @keyframes spPulse{0%,100%{box-shadow:0 8px 32px rgba(0,0,0,0.6),0 0 20px rgba(0,210,100,0.12);}50%{box-shadow:0 8px 32px rgba(0,0,0,0.6),0 0 32px rgba(0,210,100,0.25);}}
  #cbSignalPopup.buy-signal.visible{animation:spSlideIn 0.35s cubic-bezier(0.34,1.56,0.64,1) forwards,spPulse 2s ease-in-out 0.35s infinite;}
  @keyframes spPulseSell{0%,100%{box-shadow:0 8px 32px rgba(0,0,0,0.6),0 0 20px rgba(220,38,38,0.12);}50%{box-shadow:0 8px 32px rgba(0,0,0,0.6),0 0 32px rgba(220,38,38,0.25);}}
  #cbSignalPopup.sell-signal.visible{animation:spSlideIn 0.35s cubic-bezier(0.34,1.56,0.64,1) forwards,spPulseSell 2s ease-in-out 0.35s infinite;}
  ...
  <div id="cbPanel">
    ...
    <div id="cbScrollArea">
      ...
      <div class="cb-ind-row">
        <span class="cb-ind-lbl">التوقّع</span>
        <span class="cb-ind-val" id="cbForecastVal">–</span>
        <span class="cb-ind-badge" id="cbForecastBadge">pWin</span>
      </div>
      <div class="cb-ind-row">
  ...
  <!-- ═══ SUPREME-PRED V2 V13 — إشعار إشارة بنمط «احسنت» ═══ -->
  <div class="sp-overlay" id="cbSpOverlay"></div>
  <div id="cbSignalPopup">
    <div class="sp-title-bar">
      <span class="sp-title-icon">🔥</span>
      <span>SUPREME-PRED V2 V13</span>
      <button id="cbSpCloseX" style="position:absolute;left:8px;top:50%;transform:translateY(-50%);background:none;border:none;color:#9fb2c0;font-size:16px;cursor:pointer;line-height:1;padding:2px 4px;opacity:0.7;">✕</button>
    </div>
    <div class="sp-header" id="cbSpHeader">
      <div class="sp-dir-icon" id="cbSpIcon">📈</div>
      <div class="sp-dir-text" id="cbSpDir">شراء</div>
      <div class="sp-pattern" id="cbSpPattern">–</div>
    </div>
    <div class="sp-body">
      <div class="sp-row">
        <span class="sp-row-label"><span class="sp-icon">💰</span> سعر الدخول</span>
        <span class="sp-row-value green" id="cbSpPrice">0.00000</span>
      </div>
      <div class="sp-row">
        <span class="sp-row-label"><span class="sp-icon">⏱</span> عمر الصفقة</span>
        <span class="sp-row-value yellow" id="cbSpDuration">0 ث</span>
      </div>
      <div class="sp-row">
        <span class="sp-row-label"><span class="sp-icon">🕐</span> وقت الإغلاق</span>
        <span class="sp-row-value cyan" id="cbSpCloseTime">–:–:–</span>
      </div>
      <div class="sp-row">
        <span class="sp-row-label"><span class="sp-icon">⏳</span> العد التنازلي</span>
        <span class="sp-timer-badge buy" id="cbSpCountdown">0 ث</span>
      </div>
      <div class="sp-countdown-bar">
        <div class="sp-countdown-fill" id="cbSpCountdownFill" style="width:100%"></div>
      </div>
      <div class="sp-conf-row">
        <span class="sp-conf-label">🎯 الثقة</span>
        <div class="sp-conf-track">
          <div class="sp-conf-fill high" id="cbSpConfFill" style="width:75%"></div>
        </div>
        <span class="sp-conf-val" id="cbSpConfVal">75%</span>
      </div>
      <div class="sp-row">
        <span class="sp-row-label"><span class="sp-icon">📊</span> الزوج</span>
        <span class="sp-row-value" id="cbSpAsset">–</span>
      </div>
    </div>
    <div class="sp-footer">
      <button class="sp-close-btn" id="cbSpCloseBtn">✕ إغلاق</button>
    </div>
  </div>
  `;
  ...
  let _fcLastHealthLog = 0;     // [V26] خنق تكرار سجل صحة الصفقة (TickForecast)
  // [V26] تحديث خانة التوقّع في الـHUD (احتمال فوز الصفقة المفتوحة لحظياً)
  function _fcUpdateHealthHUD(h) {
    try {
      const el = W.document.getElementById('cbForecastVal');
      if (!el || !h) return;
      const pct = Math.round((h.pWin || 0) * 100);
      el.textContent = pct + '% ' + (h.itm ? '▲ITM' : '▼OTM');
      el.style.color = h.pWin >= 0.55 ? '#00d264' : (h.pWin <= (CFG.FC_HEALTH_DANGER || 0.40) ? '#ff3755' : '#ffb020');
    } catch(_) {}
  }
  ...
  function showSignalPopup(opts) {
    ...
    const popup   = W.document.getElementById('cbSignalPopup');
    if (!popup) return;
    ...
    // ─── تحديث شريط العنوان SUPREME-PRED V2 V13 ───
    const titleBar = popup.querySelector('.sp-title-bar');
    if (titleBar) {
      const dirLabel = isBuy ? '▲ CALL' : '▼ PUT';
      titleBar.innerHTML = '<span class="sp-title-icon">🔥</span><span>SUPREME-PRED V2 V13</span><span style="margin-right:auto;font-size:8px;color:' + (isBuy ? '#46d98e' : '#DC2626') + ';">' + dirLabel + '</span>';
    }

    // ─── تنسيق الرأس ───
    popup.className = isBuy ? 'buy-signal' : 'sell-signal';

    const header = W.document.getElementById('cbSpHeader');
    if (header) header.className = 'sp-header ' + (isBuy ? 'buy-bg' : 'sell-bg');

    const icon = W.document.getElementById('cbSpIcon');
    if (icon) icon.textContent = isBuy ? '📈' : '📉';

    const dirEl = W.document.getElementById('cbSpDir');
    if (dirEl) { dirEl.textContent = isBuy ? 'شراء ↑' : 'بيع ↓'; dirEl.className = 'sp-dir-text ' + (isBuy ? 'buy-text' : 'sell-text'); }

    const patEl = W.document.getElementById('cbSpPattern');
    // ✅ [INTERVAL] ترجمة اسم النمط للعربية
    const _patNames = { 'interval_signal': 'إشارة فاصل 7ث', 'uhnf_arb': '⚡ تحكيم WSS', 'tick_pulse': 'نبضة تيك', '3bullish': '3 شموع صاعدة', '3bearish': '3 شموع هابطة', 'bullish_engulfing': 'ابتلاع صاعد', 'bearish_engulfing': 'ابتلاع هابط', 'doji_reversal': 'انعكاس دوچي', 'momentum_continuation': 'استمرار زخم' };
    const _patLabel = _patNames[opts.pattern] || opts.pattern || '—';
    if (patEl) patEl.textContent = 'النمط: ' + _patLabel;

    // ─── سعر الدخول ───
    const priceEl = W.document.getElementById('cbSpPrice');
    if (priceEl) { priceEl.textContent = (opts.price || 0).toFixed(5); priceEl.className = 'sp-row-value ' + (isBuy ? 'green' : 'red'); }

    // ─── مدة الصفقة (تنسيق ذكي: ثواني أو دقائق) ───
    const durEl = W.document.getElementById('cbSpDuration');
    if (durEl) durEl.textContent = durSec >= 60 ? Math.floor(durSec/60) + ' د ' + (durSec%60 ? durSec%60 + 'ث' : '') : durSec + ' ثانية';

    // ─── وقت الإغلاق ───
    const closeMs = opts.closeAtMs || (Date.now() + durSec * 1000);
    _spCloseAtMs  = closeMs;
    _spTotalDurMs = durSec * 1000;
    const closeTime = new Date(closeMs);
    const closeTimeStr = closeTime.getHours().toString().padStart(2,'0') + ':' +
                         closeTime.getMinutes().toString().padStart(2,'0') + ':' +
                         closeTime.getSeconds().toString().padStart(2,'0');
    const closeEl = W.document.getElementById('cbSpCloseTime');
    if (closeEl) closeEl.textContent = closeTimeStr;

    // ─── العد التنازلي ───
    const countEl = W.document.getElementById('cbSpCountdown');
    const fillEl  = W.document.getElementById('cbSpCountdownFill');
    if (fillEl) fillEl.className = 'sp-countdown-fill' + (isBuy ? '' : ' sell-fill');

    // ✅ تحديث شارة العد التنازلي بلون الاتجاه
    if (countEl) {
      countEl.className = 'sp-timer-badge ' + (isBuy ? 'buy' : 'sell');
    }
    ...
    _spCountdownTimer = _v11_setInterval(() => {
      ...
      // ✅ تنسيق ذكي: ثواني فقط أو دقائق:ثواني
      if (countEl) {
        if (remainSec >= 60) {
      ...
      if (remain <= 0) {
        if (_spCountdownTimer) { clearInterval(_spCountdownTimer); _spCountdownTimer = null; }
        // ✅ لا نغلق فوراً — ننتظر الثانية الإضافية (duration+1)
        // العد التنازلي وصل صفر — الإغلاق التلقائي سيتولى الباقي
      }
    }, 250);

    // ─── الثقة ───
    const conf = opts.confidence || 0;
    const confFill = W.document.getElementById('cbSpConfFill');
    if (confFill) {
      confFill.style.width = conf + '%';
      confFill.className = 'sp-conf-fill ' + (conf >= 80 ? 'high' : conf >= 60 ? 'mid' : 'low');
    }
    const confValEl = W.document.getElementById('cbSpConfVal');
    if (confValEl) confValEl.textContent = conf + '%';

    // ─── الزوج ───
    const assetEl = W.document.getElementById('cbSpAsset');
    if (assetEl) assetEl.textContent = opts.asset || activeAsset || '—';

    // ─── عرض السلايدر (انزلاق من اليمين) ───
    ...
    addLog('🔔 [SLIDER] SUPREME-PRED V2 V13 | ' + (isBuy ? '▲ شراء' : '▼ بيع') +
           ' | ' + (opts.price || 0).toFixed(5) + ' | ' + durSec + 'ث | ثقة ' + conf +
           '% | إغلاق تلقائي بعد ' + (durSec + 1) + 'ث', 'signal');

    // صوت تنبيه (اختياري — vibration على الموبايل)
    try { if (W.navigator.vibrate) W.navigator.vibrate([80, 40, 80]); } catch(_) {}
  }
  ...
  function _initSignalPopup() {
    const closeBtn = W.document.getElementById('cbSpCloseBtn');
    if (closeBtn) closeBtn.addEventListener('click', hideSignalPopup);
    // ✅ زر ✕ في شريط العنوان (بنمط «احسنت»)
    const closeX = W.document.getElementById('cbSpCloseX');
    if (closeX) closeX.addEventListener('click', hideSignalPopup);
    // اختصار لوحة المفاتيح: Escape لإغلاق السلايدر
    ...
    if (popupToggle) {
      ...
      popupToggle.addEventListener('change', () => {
        ...
        addLog('🔔 [SLIDER] سلايدر SUPREME-PRED: ' + (_popupEnabled ? 'مفعّل ✅' : 'معطّل ❌'), 'info');
      });
    }
  }
...
;
  ...
  const DualWSSManager = (() => {
    ...
    function getAdaptiveCooldown() {
      ...
      const durMs = (_snapTradeDuration(_lastSmartDurSec >= 10 ? _lastSmartDurSec : (_tradeDuration || candlePeriod || 10))) * 1000;
    ...
    function _executeGhostTrade(direction, asset, amount) {
      ...
      const tradeSec = _snapTradeDuration(_lastSmartDurSec >= 10 ? _lastSmartDurSec : (_tradeDuration || (candlePeriod || 10)));
    ...
    function tickPulse(asset) {
      ...
      if (CFG.FC_ENABLED) {
        ...
        } else if (CFG.FC_GATE_ENABLED && CFG.FC_GATE_BLOCK) {
          addLog('🛑 [FORECAST-GATE] ' + dir + ' مرفوض — تيكات غير كافية للحساب (' + (_fc && _fc.reason || '?') + ')', 'info');
          return;
        }
      }

      // ─── [V14] تنفيذ مباشر WSS→EXEC — لا _processSignal، لا فلاتر ───
      const _smartDur = _fcDur || _computeSignalDuration(conf, 10, 'tick_pulse');
      _lastSmartDurSec = _smartDur;
      ...
      if (autoTrade) {
        ...
        if (tradeWSOrig && tradeWS && tradeWS.readyState === 1 && !_shouldBlockSend(dir, a)) {
          try {
            tradeWSOrig((action === 'call' ? _payloadCache.prefixCall : _payloadCache.prefixPut) + _nextReqId() + (action === 'call' ? _payloadCache.suffixCall : _payloadCache.suffixPut));
            tradeExec = true;
            lastTradeMs = Date.now();
            _openTradesInFlight++;
            _inFlightDirection = dir;
            if (_tradeExecTimeout) clearTimeout(_tradeExecTimeout);
            _tradeExecTimeout = setTimeout(() => {
              if (tradeExec) { tradeExec = false; _openTradesInFlight = 0; _inFlightDirection = null; updateTradeBtn(); }
            }, (tradeSec + 5) * 1000);
            _pendingTradeRecord = { asset: a, direction: dir, amount: amt, openTs: Date.now(), source: 'tickPulse_direct' };
            try { const _tb = tickBuffers[a]; _pendingTradeRecord.openPrice = (_tb && _tb.length) ? _tb[_tb.length - 1] : 0; } catch(_) {}
            addLog('⚡ [PULSE-EXEC] ' + dir + ' | ' + a + ' | $' + amt + ' | ' + tradeSec + 'ث | مباشر WSS→EXEC', 'signal');
            updateTradeBtn();
          } catch(err) {
            addLog('❌ [PULSE] فشل إرسال: ' + err.message, 'error');
          }
        } else if (!tradeWSOrig || !tradeWS || tradeWS.readyState !== 1) {
          addLog('⚠️ [PULSE] لا يوجد مقبس — تخطّي', 'error');
        }
      } else {
    ...
    function _processSignal(signal, opts) {
      ...
      // ═══ [V26] بوابة الإسقاط الأمامي الموحّدة — عملية حسابية دقيقة لكل الأنماط ═══
      //   نحسب pWin/EV/أفضل مدة؛ نرفض الخاسر إحصائياً (عدا إشارة الفاصل — استشاري فقط).
      if (CFG.FC_ENABLED && CFG.FC_GATE_ENABLED) {
        const _fcA = normalizeAsset(signal.asset || activeAsset);
        const _fcS = TickForecast.decide(_fcA, signal.direction);
        if (_fcS && _fcS.pWin != null) {
          signal._fcPWin = _fcS.pWin; signal._fcEV = _fcS.ev;
          if (CFG.FC_DURATION_ENABLED && _fcS.durSec) { signal._fcDur = _fcS.durSec; _lastSmartDurSec = _fcS.durSec; }
          if (CFG.FC_GATE_BLOCK && !_fcS.ok && !_isInterval) {
            addLog('🛑 [FORECAST-GATE] ' + signal.direction + ' (' + signal.pattern + ') مرفوض — pWin ' +
                   Math.round(_fcS.pWin * 100) + '% / EV ' + _fcS.ev.toFixed(2) + ' بلا حافة إحصائية', 'info');
            return;
          }
          addLog('🔮 [FORECAST] ' + signal.direction + ' (' + signal.pattern + ') pWin ' + Math.round(_fcS.pWin * 100) +
                 '% EV ' + (_fcS.ev >= 0 ? '+' : '') + _fcS.ev.toFixed(2) + ' مدة ' + _fcS.durSec + 'ث', _fcS.ok ? 'signal' : 'info');
        }
      }
    ...
    function _eteMaxWait() {
      ...
      const durSec = _lastSmartDurSec >= 10
        ? _snapTradeDuration(_lastSmartDurSec)
        : _snapTradeDuration(_tradeDuration || (candlePeriod || 10));
