// ==UserScript==
// @name         ⚡ V13.1_QUANTUM_ENGINE — Dual-Node Latency Arbitrage (Oracular/Executor)
// @namespace    candle-pro-strategy-v13-quantum-engine
// @version      13.7.0
// @description  QUANTUM SKELETON — Dual-Node (US-Oracular / KSA-Executor) wrapper over existing pattern engine + raw-frame WS interception + UI
// @author       aoirusra
...
(function (W) {
  ...
  const CFG = {
    ...
    // ─── Dual-Node Latency Arbitrage (Oracular US ↔ Executor KSA) ────
    //   عقدتان منطقيتان داخل سياق واحد، يربطهما ناقل أحداث يحاكي قفزة شبكية.
    //   ملاحظة صدق: العقدتان تراقبان نفس المقبس فلا مراجحة كمون حقيقية ضد
    //   المنصة — القفزة محاكاة للعزل والقياس فقط. الأساس=0 يبقي الدخول فورياً
    //   (مطابقاً لسلوك V24-FAST)؛ ارفعه فقط لاختبار سلوك الكمون.
    DN_ENABLED              : true,       // تفعيل غلاف العقدتين
    DN_HOP_BASE_MS          : 0,          // كمون القفزة الأساسي المحاكى (0 = فوري)
    DN_HOP_JITTER_MS        : 0,          // أقصى اهتزاز عشوائي يُضاف للقفزة
    DN_CLOCK_EMA_ALPHA      : 0.15,       // معامل EMA لمزامنة الساعة US↔KSA
    DN_CLOCK_BASE_OFFSET_MS : 0,          // إزاحة ساعة أولية مقدّرة (US−KSA)
    DN_CLOCK_MAX_SAMPLE_MS  : 3000,       // تجاهل عيّنات الإزاحة الشاذة فوق هذا
    DN_PREDICT_SLOPE_MS     : 800,        // نافذة الميل لحساب «الإغلاق المستقبلي»
    DN_STATS_LOG_MS         : 15000,      // فاصل تسجيل إحصاءات الكمون/الانجراف
  ...
  function handleBinaryFrame(rawData, urlStr, wsRef) {
    ...
    toBuffer.then(buf => {
      ...
      // [V7] Oracular-Node: مراقبة الإطار الخام (Uint8Array) قبل طبقة الواجهة —
      //   تأطير Socket.IO/MsgPack + عيّنة ساعة الأوراكل. لا قرار هنا، قياس فقط.
      if (CFG.DN_ENABLED) { try { OracularNode.observeFrame(new Uint8Array(buf), wsRef && wsRef._dualRole); } catch (_) {} }
      let evName = 'binary';
  ...
  function onOpenOrderSuccess(data) {
    ...
    const durSec  = (openTs && closeTs) ? (closeTs - openTs) : (_snapTradeDuration(_tradeDuration || candlePeriod || 3));
...
;
  ...
  // ══════════════════════════════════════════════════════════════════════
  // § DN  Dual-Node Latency Arbitrage Architecture
  //   نموذج «عقدتين منطقيتين» داخل سياق تنفيذ واحد (سكربت موحّد):
  //     • Oracular-Node (US-Proxy)   — «المراقب الأساسي»: يعترض الإطارات الخام
  //       (Uint8Array) قبل طبقة الواجهة، يشغّل كشف الأنماط القائم، يحسب
  //       «الإغلاق المستقبلي» (تنبؤ)، ثم يطلق «نية تنفيذ» عبر الناقل.
  //     • Executor-Node (KSA-Target) — «معالج التنفيذ»: يستمع لأحداث الناقل،
  //       يطبّق إزاحة مزامنة الساعة + تصحيح الانجراف، ثم ينفّذ عبر مدير المقابس.
  //   الناقل الداخلي (MessagePort إن توفّر، وإلا EventEmitter) يحاكي قفزة
  //   شبكية عبر القارات بكمون/اهتزاز قابلين للضبط.
  //
  //   ⚠️ ملاحظة صدق معماري: العقدتان تعملان في نفس السياق وتراقبان نفس
  //   المقبس، فلا توجد *مراجحة كمون حقيقية* تسبق المنصة — القفزة محاكاة
  //   لغرض العزل والقياس وصحة المسار. الأساس الافتراضي = 0ms يبقي التنفيذ
  //   فورياً (مطابقاً لسلوك V24-FAST). المنطق المضاف غلاف تنظيمي + قياس،
  //   لا يغيّر منطق كشف الأنماط القائم.
  // ══════════════════════════════════════════════════════════════════════

  // ─── الناقل الداخلي: حلقة وصل العقدتين (MessagePort/EventEmitter) ───────
  const NodeEventBus = (() => {
    const _listeners = new Map();          // channel -> Set<fn>
    let _seq = 0;
    let _port1 = null, _port2 = null;      // قناة محاكاة قفزة الشبكة
    function _mono() { return (W.performance && W.performance.now) ? W.performance.now() : Date.now(); }
    function _dispatch(packet) {
      packet.recvTs = _mono();             // طابع الوصول عند عقدة المنفّذ (مونوتوني)
      const set = _listeners.get(packet.channel);
      if (!set) return;
      for (const fn of set) { try { fn(packet); } catch (e) { try { addLog('❌ [BUS] ' + e.message, 'error'); } catch (_) {} } }
    }
    try {
      if (typeof MessageChannel === 'function') {
        const mc = new MessageChannel();
        _port1 = mc.port1; _port2 = mc.port2;
        _port2.onmessage = (ev) => _dispatch(ev.data);
        if (_port1.start) _port1.start();
        if (_port2.start) _port2.start();
      }
    } catch (_) { _port1 = _port2 = null; }
    function on(channel, fn) {
      if (!_listeners.has(channel)) _listeners.set(channel, new Set());
      _listeners.get(channel).add(fn);
      return () => off(channel, fn);
    }
    function off(channel, fn) { const s = _listeners.get(channel); if (s) s.delete(fn); }
    // إرسال «نية» عبر الناقل مع كمون قفزة محاكى (يعبر حدّ async حقيقياً)
    function emit(channel, payload, hopMs) {
      const packet = { channel, payload, seq: ++_seq, sendTs: _mono(), hopMs: hopMs | 0 };
      const deliver = () => {
        if (_port1) { try { _port1.postMessage(packet); return; } catch (_) {} }
        _dispatch(packet);                 // مسار احتياطي إن غاب MessageChannel
      };
      if (hopMs > 0) setTimeout(deliver, hopMs); else deliver();
      return packet.seq;
    }
    return { on, off, emit, now: _mono, hasPort: () => !!_port1 };
  })();

  // ─── مزامنة الساعة: إزاحة US↔KSA + تصحيح الانجراف ──────────────────────
  const ClockSync = (() => {
    let _offset = CFG.DN_CLOCK_BASE_OFFSET_MS || 0;   // إزاحة مقدّرة (KSA − US)
    let _drift  = 0;                                  // معدل تغيّر الإزاحة
    let _samples = 0;
    let _lastOracleTs = 0;
    // عيّنة وصول حزمة لعقدة الأوراكل (بيئة US)
    function sampleOracle(ts) { _lastOracleTs = ts; }
    // عيّنة وصول حزمة لعقدة المنفّذ (بيئة KSA) — تحدّث الإزاحة بـ EMA + الانجراف
    function sampleExecutor(ts) {
      if (_lastOracleTs <= 0) return;
      const raw = ts - _lastOracleTs;
      if (Math.abs(raw) >= (CFG.DN_CLOCK_MAX_SAMPLE_MS || 3000)) return;
      const a = CFG.DN_CLOCK_EMA_ALPHA || 0.15;
      const prev = _offset;
      _offset = _samples === 0 ? raw : (_offset * (1 - a) + raw * a);
      _drift  = _drift * (1 - a) + (_offset - prev) * a;
      _samples++;
    }
    function getOffset() { return _offset; }
    function getDrift()  { return _drift; }
    function getSamples() { return _samples; }
    // العمر الفعلي لإشارة بعد طرح إزاحة الساعة (تصحيح الانجراف)
    function correctedAge(oracleTs, nowExecTs) { return (nowExecTs - oracleTs) - _offset; }
    return { sampleOracle, sampleExecutor, getOffset, getDrift, getSamples, correctedAge };
  })();

  // ─── إحصاءات الكمون + تسجيل تصحيح الانجراف ─────────────────────────────
  const LatencyArbStats = (() => {
    const _hops = []; const MAX = 200;
    let intentsFired = 0, intentsDelivered = 0, intentsExpired = 0;
    let _lastLogTs = 0;
    function recordHop(ms) { _hops.push(ms); if (_hops.length > MAX) _hops.shift(); }
    function _pct(arr, p) { if (!arr.length) return 0; const s = [...arr].sort((a, b) => a - b); return s[Math.min(s.length - 1, Math.floor(p * s.length))]; }
    function summary() {
      const n = _hops.length;
      const avg = n ? _hops.reduce((s, x) => s + x, 0) / n : 0;
      let jitter = 0; if (n) jitter = Math.sqrt(_hops.reduce((s, x) => s + (x - avg) * (x - avg), 0) / n);
      return {
        n, avg, min: n ? Math.min(..._hops) : 0, max: n ? Math.max(..._hops) : 0,
        p95: _pct(_hops, 0.95), jitter,
        offset: ClockSync.getOffset(), drift: ClockSync.getDrift(), syncSamples: ClockSync.getSamples(),
        intentsFired, intentsDelivered, intentsExpired,
      };
    }
    function onFired()     { intentsFired++; }
    function onDelivered() { intentsDelivered++; }
    function onExpired()   { intentsExpired++; }
    function maybeLog() {
      const now = Date.now(), iv = CFG.DN_STATS_LOG_MS || 15000;
      if (now - _lastLogTs < iv) return;
      _lastLogTs = now;
      const s = summary();
      if (s.n === 0 && s.intentsFired === 0) return;
      addLog('📊 [LAT-ARB] قفزة الناقل: متوسط ' + s.avg.toFixed(1) + 'ms | أدنى ' + s.min.toFixed(1) +
             ' | أقصى ' + s.max.toFixed(1) + ' | p95 ' + s.p95.toFixed(1) + ' | اهتزاز ' + s.jitter.toFixed(1) + 'ms', 'info');
      addLog('📊 [DRIFT] إزاحة الساعة US↔KSA: ' + s.offset.toFixed(1) + 'ms | انجراف ' + s.drift.toFixed(2) +
             'ms | عيّنات مزامنة: ' + s.syncSamples, 'info');
      addLog('📊 [INTENTS] أُطلقت ' + s.intentsFired + ' | سُلّمت ' + s.intentsDelivered + ' | انتهت ' + s.intentsExpired, 'info');
    }
    return { recordHop, summary, onFired, onDelivered, onExpired, maybeLog };
  })();

  // ─── Oracular-Node (US-Proxy): المراقب الأساسي + التنبؤ + إطلاق النية ──
  const OracularNode = (() => {
    const NODE_ID = 'US-ORACULAR';
    let _frames = 0;
    // مراقبة الإطار الخام (Uint8Array) قبل طبقة الواجهة — تأطير Socket.IO/MsgPack
    function observeFrame(bytes, role) {
      if (!bytes || !bytes.length) return;
      _frames++;
      // عيّنة ساعة الأوراكل عند كل إطار خام يصل من مقبس US (events-po)
      if (role === 'oracle') ClockSync.sampleOracle(Date.now());
    }
    // حساب «الإغلاق المستقبلي» (تنبؤ) من حالة التيك القائمة + الإشارة
    function _predictFutureClose(signal) {
      try {
        const a = normalizeAsset(signal.asset);
        const tb = tickBuffers[a];
        const last = (tb && tb.length) ? tb[tb.length - 1] : (signal.price || 0);
        const sl = OracleLab.microSlope(a, CFG.DN_PREDICT_SLOPE_MS || 800);
        const horizonSec = _snapTradeDuration(_tradeDuration || candlePeriod || 3);
        const slopeRel = sl ? sl.rel : 0;
        const projected = last * (1 + slopeRel * horizonSec);   // إسقاط خطّي بسيط
        return { last, projected, horizonSec, slopeRel };
      } catch (_) { return { last: signal.price || 0, projected: signal.price || 0, horizonSec: 0, slopeRel: 0 }; }
    }
    function _computeHop() {
      const base = CFG.DN_HOP_BASE_MS || 0;
      const jit = CFG.DN_HOP_JITTER_MS ? Math.round(Math.random() * CFG.DN_HOP_JITTER_MS) : 0;
      return Math.max(0, base + jit);
    }
    // تغليف الإشارة كـ«نية تنفيذ» وإطلاقها عبر الناقل إلى عقدة المنفّذ
    function fireIntent(signal) {
      const predict = _predictFutureClose(signal);
      const hop = _computeHop();
      let gap = 0; try { gap = DualWSSManager.getLatencyGap(); } catch (_) {}
      const intent = { node: NODE_ID, signal, predict, oracleTs: Date.now(), gap };
      LatencyArbStats.onFired();
      addLog('🛰️ [ORACULAR] نية تنفيذ ' + signal.direction + ' | ' + signal.asset +
             ' | تنبؤ الإغلاق ' + predict.projected.toFixed(5) + ' (أفق ' + predict.horizonSec + 'ث) | قفزة محاكاة ' + hop + 'ms', 'signal');
      NodeEventBus.emit('EXECUTION_INTENT', intent, hop);
    }
    return { observeFrame, fireIntent, NODE_ID, getFrames: () => _frames };
  })();

  // ─── Executor-Node (KSA-Target): استقبال النية + تصحيح + تنفيذ ─────────
  const ExecutorNode = (() => {
    const NODE_ID = 'KSA-EXECUTOR';
    let _sink = null, _bound = false;
    function bindSink(fn) { _sink = fn; }     // يربطه المدير بخط أنابيب التنفيذ
    function _onIntent(packet) {
      const intent = packet.payload;
      const recvTs = Date.now();              // ساعة بيئة المنفّذ (KSA)
      ClockSync.sampleExecutor(recvTs);
      const hop = packet.recvTs - packet.sendTs;   // كمون القفزة الفعلي (مونوتوني)
      LatencyArbStats.recordHop(hop);
      const rawAge  = recvTs - intent.oracleTs;
      const corrAge = ClockSync.correctedAge(intent.oracleTs, recvTs);
      if (corrAge > (CFG.DUAL_WSS_MAX_SIGNAL_AGE || 2000)) {
        LatencyArbStats.onExpired();
        addLog('⌛ [EXECUTOR] نية منتهية — عمر مصحّح ' + corrAge.toFixed(0) + 'ms (خام ' + rawAge + 'ms) > الحد', 'info');
        return;
      }
      if (!_sink) { addLog('⚠️ [EXECUTOR] لا مصرف تنفيذ مربوط', 'error'); return; }
      addLog('⚡ [EXECUTOR] استلام نية ' + intent.signal.direction + ' | قفزة ' + hop.toFixed(1) +
             'ms | عمر مصحّح ' + corrAge.toFixed(0) + 'ms | seq#' + packet.seq, 'signal');
      LatencyArbStats.onDelivered();
      _sink(intent.signal);                   // تسليم إلى خط أنابيب الحماية+الإرسال القائم
    }
    function start() {
      if (_bound) return;
      NodeEventBus.on('EXECUTION_INTENT', _onIntent);
      _bound = true;
      addLog('⚡ [EXECUTOR] عقدة KSA متصلة بالناقل (' + (NodeEventBus.hasPort() ? 'MessagePort' : 'EventEmitter') + ')', 'signal');
    }
    return { start, bindSink, NODE_ID };
  })();
  ...
  // [V7] أصبح خط الأنابيب الخلفي لعقدة المنفّذ: كشف الأنماط ← أوراكل ← ناقل
  //      ← منفّذ ← حُرّاس _processQueue القائمون ← إرسال المقبس.
  // ══════════════════════════════════════════════════════════════════════
  ...
  const DualWSSManager = (() => {
    ...
    let _latStatsTimer  = null;   // [V7] مؤقت تسجيل إحصاءات الكمون/الانجراف
    ...
    function init() {
      ...
      _running = true;
      // [V7] تشغيل غلاف العقدتين: اربط مصرف المنفّذ بخط الأنابيب وفعّل الاستماع
      if (CFG.DN_ENABLED) {
        ExecutorNode.bindSink(_deliverIntentToPipeline);
        ExecutorNode.start();
        if (!_latStatsTimer) _latStatsTimer = setInterval(() => { try { LatencyArbStats.maybeLog(); } catch (_) {} }, Math.max(2000, (CFG.DN_STATS_LOG_MS || 15000) / 3));
        addLog('🛰️ [DUAL-NODE] العقدتان فعّالتان — Oracular(US) → ناقل → Executor(KSA)', 'signal');
      }
      _refreshDualWSSHUD();
    }
    ...
    function recordMsgTs(role) {
      ...
      if (role === 'oracle') {
        _oracleMsgTs = now;
        // [V7] عيّنة ساعة الأوراكل (US) لمزامنة الساعة بين العقدتين
        if (CFG.DN_ENABLED) { try { ClockSync.sampleOracle(now); } catch (_) {} }
      } else if (role === 'executor') {
        _executorMsgTs = now;
        // [V7] عيّنة ساعة المنفّذ (KSA) → تحديث إزاحة الساعة + الانجراف
        if (CFG.DN_ENABLED) { try { ClockSync.sampleExecutor(now); } catch (_) {} }
        if (_oracleMsgTs > 0) {
    ...
    function _enqueueSignal(signal) {
      ...
      // علامة «مشغول» متزامنة (يستخدمها مولّد PSE: `if (_signalQueue || tradeExec)`)
      //   — تبقى محفوظة رغم تمرير الإشارة عبر الناقل (الذي قد يؤخّر التسليم).
      _signalQueue = signal;
      _lastSignal = signal;
      ...
      // [V7] كشف الأنماط القائم انتهى → سلّم الإشارة لعقدة الأوراكل التي تطلق
      //   «نية تنفيذ» عبر الناقل. عقدة المنفّذ تُعيدها إلى _processQueue (الحُرّاس
      //   + الإرسال) عبر المصرف المربوط في init(). إن تعطّل الغلاف نرجع للمسار المباشر.
      if (CFG.DN_ENABLED) {
        OracularNode.fireIntent(signal);
      } else {
        _processQueue();   // المسار المباشر القديم (الغلاف معطّل)
      }
    }
    // [V7] مصرف عقدة المنفّذ → خط أنابيب التنفيذ القائم (نفس الحُرّاس تماماً)
    function _deliverIntentToPipeline(signal) {
      _signalQueue = signal;
      _processQueue();
    }
    ...
    function shutdown() {
      ...
      if (_entryTimer) { clearInterval(_entryTimer); _entryTimer = null; }
      if (_latStatsTimer) { clearInterval(_latStatsTimer); _latStatsTimer = null; }
      _signalQueue = null;
