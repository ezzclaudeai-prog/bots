// ==UserScript==
// @name         ⚡ V13.4_RAW_SCALP — وضع خام بلا فلاتر (سكالبينغ سريع — لقياس خط الأساس)
// @namespace    candle-pro-strategy-v13-quantum-engine
// @version      13.4.0-RAW
// @description  RAW MODE — كل الفلاتر مُعطّلة، دخول فوري. للقياس فقط (كم ربح/كم خسارة). مخاطرة أعلى.
// @author       aoirusra
...
(function (W) {
  ...
  const CFG = {
    ...
    DUAL_WSS_SYNTHETIC_DELAY : 0,        // [RAW] بلا تأخير اصطناعي — دخول فوري
    ...
    DUAL_WSS_JITTER_MS      : 0,        // [RAW] بلا اهتزاز — دخول فوري

    // ─── حماية متقدمة ──────────────────────────────────────────────
    MIN_CONFIDENCE_THRESHOLD: 50,        // [RAW] أدنى عتبة ثقة (السلايدر) — خام
    GHOST_TRADE_ENABLED     : false,        // [RAW] لا صفقات وهمية
    ...
    RECALIBRATE_ON_STREAK   : 9999,        // [RAW] لا إعادة معايرة
    ...
    CANDLE_LOCK_ENABLED     : false,        // [RAW] السماح بأكثر من صفقة في نفس الشمعة (أسرع)
    TRADE_COOLDOWN_RATIO    : 0,        // [RAW] بلا تهدئة
    TRADE_COOLDOWN_FLOOR_MS : 0,        // [RAW] بلا تهدئة
    LOSS_STREAK_PAUSE_MS    : 0,        // [RAW] لا وقف بعد الخسائر

    // ─── حماية استنفاد الاتجاه وتكرار النمط ──────────────────────────────
    EXHAUSTION_ENABLED      : false,        // [RAW] لا كشف استنفاد
    ...
    TREND_SOFT_PENALTY      : 0,        // [RAW] بلا خصم اتجاه
    ...
    ADAPTIVE_CONF_ENABLED   : false,        // [RAW] لا ثقة تكيفية
    ...
    ORACLE_CONFIRM_ENABLED  : false,        // [RAW] لا فلتر أوراكل
    ...
    FAST_EVAL_MS            : 300,        // [RAW] تقييم داخل الشمعة أسرع
    FAST_EVAL_MIN_TICKS     : 4,           // أدنى عدد تيكات في الشمعة المتشكّلة قبل تقييمها
    FAST_EVAL_MIN_CONF      : 0,        // [RAW] لا عتبة للتقييم السريع
    // ─── [V17] محرّك توقيت الدخول (ETE) — لا تدخل إلا حين يوافق الزخم اللحظي ──
    ENTRY_TIMING_ENABLED    : false,        // [RAW] دخول فوري بلا انتظار توقيت ETE
    ETE_SLOPE_MS            : 2500,        // ✅ [V3-MERGE/FIX] التيك ~470ms → 1200ms كان تيكين فقط. 2500ms ≈ 5 تيكات = ميل موثوق
    ETE_MIN_REL             : 0.000020,   // أدنى عائد نسبي ليُعدّ الميل اتجاهاً (وإلا «مسطّح»)
    ETE_MIN_TICKS           : 3,           // ✅ [V3-MERGE/FIX] أدنى عدد تيكات لاعتبار الميل اتجاهاً حقيقياً (وإلا «مسطّح» → ينتظر)
    ...
    ETE_FLAT_WAITS          : false,        // [RAW] لا انتظار للزخم المسطّح

    // ══════════════════════════════════════════════════════════════════
    // ─── [V3-MERGE] حُرّاس V2 الدفاعية الجراحية — مطعّمة في أساس V1 الرابح 17:5 ──
    //   مشتقة من التحليل الجنائي لخسائر السجل (T6 71%, T8 72%, T9 73% معاكسة + زخم رقيق).
    //   لا تُضاف أي ميزة عدوانية (لا TICKPULSE، لا CONTINUATION، لا صفقتان فعليتان، لا دخول فوري).
    // ══════════════════════════════════════════════════════════════════
    ABSOLUTE_MIN_CONF       : 0,        // [RAW] بلا أرضية ثقة
    // ─── [FIX-E] حارس جراحي للخسائر الحدّية — بلا إبطاء (الإشارات القوية تمر فوراً) ──
    FIXE_ENABLED            : false,        // [RAW] حارس FIX-E مُعطّل
    ...
    // ─── [FIX-G] حجب أطول للشات المعاكس — M15 يبقى صالحاً ~15د لا 90ث ──
    FIXG_CHAT_VETO_ENABLED  : false,        // [RAW] FIX-G مُعطّل
    FIXG_CHAT_VETO_TTL_MS   : 900000,      // ✅ [FIX-G] ارفض أي إشارة تعاكس الشات خلال 15 دقيقة (الشات M15 كان محقاً في كل الخسائر)
    // ─── [FIX-H] استئناف أصعب بعد إعادة المعايرة — يمنع الاستئناف عند قمة ارتداد ──
    FIXH_RESUME_CANDLES     : 5,           // ✅ [FIX-H] شموع متتالية أكثر لإنهاء إعادة المعايرة (5 بدل 3)
    FIXH_RESUME_NEEDS_ORACLE: false,        // [RAW] FIX-H مُعطّل
    // ─── [FIX-B] تثبيت مدة السكالبينغ — يمنع time:60 الشاذ المرصود في السجل ──
    //   مُطفأ افتراضياً للحفاظ على سلوك V1 الرابح (يحترم مدة المستخدم). فعّله للسكالبينغ الصرف بمدة ثابتة.
    SCALP_FIXED_ENABLED     : false,       // ⚙️ [FIX-B] (اختياري) فعّله لتثبيت مدة كل صفقة على SCALP_FIXED_SEC
    SCALP_FIXED_SEC         : 3,           // [FIX-B] المدة الثابتة عند التفعيل
    SCALP_MAX_SEC           : 5,           // [FIX-B] حد أقصى عند التفعيل
    // ─── [FIX-F] إيقاف المضاعفة بعد خسائر متتالية — يوقف النزيف ──
    FIXF_NO_DOUBLE_AFTER_LOSSES : 2,       // ✅ [FIX-F] إذا lossStreak ≥ هذا → بلا مضاعفة (مبلغ أساسي فقط)
    // ─── [V3] مضاعفة المبلغ المحدودة ×2 — قرار المستخدم: ×2 كحد أقصى عند الثقة العالية ──
    DOUBLE_ON_STRONG        : false,        // [RAW] بلا مضاعفة — مبلغ ثابت لقياس نظيف
    DOUBLE_STRONG_CONF      : 85,          // ✅ [V3] عتبة الثقة للمضاعفة ×2
    DOUBLE_MAX_MULT         : 2,           // ✅ [V3] أقصى مضاعفة = ×2 (لا ×3/×4)
  ...
  function _snapTradeDuration(secs) {
    // ⚙️ [FIX-B] (اختياري) تثبيت مدة السكالبينغ — يمنع time:60 الشاذ. مُطفأ افتراضياً (يحترم مدة المستخدم).
    if (CFG.SCALP_FIXED_ENABLED) {
      ...
      return Math.max(CFG.MIN_TRADE_SEC || 3, fx);
    }
  ...
  let _minConfThreshold = 50; // حد الثقة الأدنى — يتحكم به سلايدر الواجهة
  ...
  function _shouldBlockSend(direction, asset) {
    // FIX-A: الزوج المُرسَل يجب أن يطابق الزوج النشط (تيار السعر) — يمنع تداخل الأزواج
    const a = normalizeAsset(asset || activeAsset);
    if (!a || a !== activeAsset) {
      addLog('🚫 [FIX-A] رفض — الزوج (' + a + ') لا يطابق النشط (' + activeAsset + ') | تداخل أزواج', 'error');
      return true;
    }
    // FIX-C: لا صفقة جديدة في نفس الاتجاه ما دامت صفقة سابقة مفتوحة لم تصل نتيجتها
  ...
  function processCloseOrder(data) {
    ...
    // [V24-2X] قد تُغلق أكثر من صفقة معاً في نفس الحدث — عالج كلاً على حدة
    ...
    if (win && _lastTradeWasDouble) STATS.doubleWins = (STATS.doubleWins || 0) + 1;   // ✅ [V3] عدّاد الفوز المزدوج
    if (_openTradesInFlight > 0) _openTradesInFlight--;                                // ✅ [FIX-C]
    if (_openTradesInFlight === 0) _inFlightDirection = null;                          // ✅ [FIX-C]
  ...
  function onTick(asset, price, serverTs, srcRole) {
    ...
    // ✅ [V3/V24] دقة الملي‑ثانية: استخدم توقيت الخادم (updateStream[1]) للميل اللحظي بدل التوقيت المحلي
    ...
    try { OracleLab.onTick(a, price, labTs); } catch(_) {}   // [V16+V3] التقاط خام بتوقيت الخادم الدقيق
  ...
  <div id="cbPanel">
    <div class="cb-hdr" id="cbDragHdr">
      ...
      <span class="cb-ttl" id="cbMainTitle">⚡ RAW SCALP — بلا فلاتر ⚡</span>
    ...
    <div id="cbScrollArea">
      ...
      <div class="cb-conf-slider-row" style="display:flex;align-items:center;gap:6px;padding:4px 10px;">
        ...
        <input type="range" id="cbConfSlider" min="50" max="95" value="50" style="flex:1;accent-color:#00d264;height:4px;">
        <span style="font-size:11px;font-weight:700;color:#00d264;min-width:28px;text-align:right;" id="cbConfSliderVal">50%</span>
      </div>
      ...
      <div style="display:flex;gap:6px;margin:0 10px 8px;align-items:center;">
        <input type="password" id="cbGhToken" placeholder="ghp_... GitHub Token" style="flex:1;background:#16222e;border:1px solid #243443;border-radius:8px;color:#dfe7ee;font-size:10px;padding:5px 8px;outline:none;">
    ...
    <div id="cbStatus">◆ RAW MODE ◆ كل الفلاتر OFF · دخول فوري · للقياس فقط ◆</div>
  </div>
  <div id="cbLogFloat">
    <div id="cbLogHdr">
  ...
  function initUI() {
    ...
    if (titleEl) {
      ...
      titleEl.textContent = '⚡ RAW SCALP' + (badges.length ? ' ' + badges.join('') : '');
    }
...
;
    ...
    _updateKellyDisplay();
    _updatePPTDisplay();
    addLog('🚀 RAW SCALP V13.4-RAW — كل الفلاتر مُعطّلة · دخول فوري · وضع قياس خط الأساس ⚠️', 'signal');
    updateStatsUI();
  ...
  const DualWSSManager = (() => {
    ...
    function oracleAgrees(direction, asset) {
      ...
      // ✅ [FIX-G] حجب المعاكس بنافذة أطول (M15 يبقى صالحاً ~15د) — الشات كان محقاً في كل خسائر السجل
    ...
    let _lastFastEval = 0;
    let _inFastEval = false;                    // ✅ [V3] صحيح أثناء تقييم شمعة غير مكتملة (لبوابة FAST_EVAL_MIN_CONF)
    function fastEval(asset) {
      ...
      _inFastEval = true;                      // ✅ [V3] وضع التقييم داخل الشمعة
      try { onCandleClose(a); } catch(_) {} finally { buf.pop(); _inFastEval = false; }   // قيّم ثم أزلها
    }
    ...
    function onCandleClose(asset) {
      ...
      if (signal) {
        // ✅ [V3] داخل الشمعة: لا تدخل إلا الإشارة القوية — الضعيفة تنتظر إغلاق الشمعة (أمان)
        ...
        const _effThreshold = Math.max(_minConfThreshold + _adapt.bump, CFG.ABSOLUTE_MIN_CONF || 0);  // ✅ [V3] أرضية صارمة 60%
        if (_adapt.disabled) {
        ...
        //   هذا التقاطع بالضبط هو ملف الخسائر في السجل (T6 71%, T8 72%, T9 73%). الإشارات القوية لا تتأثر.
        if (CFG.FIXE_ENABLED) {
    ...
    function _processQueue() {
      ...
      // ✅ [V3] مضاعفة المبلغ ×2 المحدودة عند الثقة العالية — صفقة واحدة بمبلغ مضاعف (لا صفقتان)
      //   FIX-F: توقف المضاعفة بعد خسائر متتالية لإيقاف النزيف. أقصى مضاعفة = DOUBLE_MAX_MULT (×2).
      let _execAmount = tradeAmount;
      _lastTradeWasDouble = false;
      const _allowDoubleF = (STATS.lossStreak || 0) < (CFG.FIXF_NO_DOUBLE_AFTER_LOSSES || 2);
      if (CFG.DOUBLE_ON_STRONG && (signal.confidence || 0) >= (CFG.DOUBLE_STRONG_CONF || 85)) {
        if (_allowDoubleF) {
          _execAmount = _safeAmount(tradeAmount * Math.min(2, CFG.DOUBLE_MAX_MULT || 2));
          _lastTradeWasDouble = true;
          STATS.doubles = (STATS.doubles || 0) + 1;
          addLog('🔥 [2X] إشارة قوية ' + signal.confidence + '% → مبلغ مضاعف ×2 = $' + _execAmount, 'signal');
        } else {
          addLog('🛡️ [FIX-F] لا مضاعفة — ' + STATS.lossStreak + ' خسائر متتالية → مبلغ أساسي فقط', 'info');
        }
      }
    ...
    function _entryAligned(asset, direction) {
      ...
      // ✅ [FIX] الميل المبني على تيك أو تيكين = ضجيج (سبب خسائر momentum/tick في السجل).
    ...
    function _timedExecute(direction, asset, amount) {
      ...
      // ✅ [V24/FIX] الزخم المسطّح لا يدخل فوراً — ينتظر ميلاً حقيقياً (كل صفقات «مسطّح» خسرت)
    ...
    function _executeDualTrade(direction, asset, overrideAmount) {
      if (!autoTrade) return;
      if (tradeExec) return;
      if (_shouldBlockSend(direction, asset)) return;   // ✅ [FIX-A/C] حارس تداخل الأزواج/تكرار الأمر
      if (!tradeWSOrig || !tradeWS || tradeWS.readyState !== 1) {
      ...
      try {
        tradeWSOrig(msg);
        tradeExec = true;
        _openTradesInFlight++; _inFlightDirection = direction;   // ✅ [FIX-C] صفقة مفتوحة في الطريق
        lastTradeMs = Date.now();
        // ✅ تحرير تلقائي لـ tradeExec بعد مدة الصفقة + 5 ثواني أمان
        if (_tradeExecTimeout) clearTimeout(_tradeExecTimeout);
        _tradeExecTimeout = setTimeout(() => {
          if (tradeExec) {
            tradeExec = false;
            _openTradesInFlight = 0; _inFlightDirection = null;   // ✅ [FIX-C] أمان: صفّر العدّاد عند المهلة
