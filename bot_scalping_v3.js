// ==UserScript==
// @name         ⚡ V13.4_QUANTUM_MERGE — V1 الرابح (17:5) + حُرّاس V2 الدفاعية + واجهة QUANTUM الداكنة
// @namespace    candle-pro-strategy-v13-quantum-engine
// @version      13.4.0
// @description  QUANTUM MERGE — أساس V1 المنضبط + FIX-A/B/C/E/G/H + ETE/أرضية ثقة + مضاعفة ×2 محدودة (FIX-F)
// @author       aoirusra
...
(function (W) {
  ...
  const CFG = {
    ...
    FAST_EVAL_MS            : 1500,        // أدنى فاصل بين تقييمين سريعين (مللي ثانية) — تمهّل V1 الرابح
    FAST_EVAL_MIN_TICKS     : 4,           // أدنى عدد تيكات في الشمعة المتشكّلة قبل تقييمها
    FAST_EVAL_MIN_CONF      : 75,          // ✅ [V3-MERGE/FIX] لا دخول سريع داخل الشمعة إلا بثقة ≥ هذه (الضعيف ينتظر الإغلاق)
    ...
    ETE_FLAT_WAITS          : true,        // ✅ [V3-MERGE/V24] الزخم المسطّح ينتظر ميلاً حقيقياً ثم يُلغى (لا يدخل فوراً على الضجيج)
    ...
    // ══════════════════════════════════════════════════════════════════
    ABSOLUTE_MIN_CONF       : 60,          // ✅ [V3] أرضية ثقة صارمة — لا صفقة تحت 60% مهما كان السلايدر
    ...
    FIXH_RESUME_NEEDS_ORACLE: true,        // ✅ [FIX-H] لا تستأنف إذا كان الشات يعارض اتجاه الاستئناف
    ...
    DOUBLE_ON_STRONG        : true,        // ✅ [V3] ضاعف المبلغ ×2 عند إشارة قوية (صفقة واحدة، لا صفقتان)
  ...
  <div id="cbPanel">
    <div class="cb-hdr" id="cbDragHdr">
      ...
      <span class="cb-ttl" id="cbMainTitle">⚡ QUANTUM MERGE V13.4 ⚡</span>
    ...
    <div id="cbStatus">◆ QUANTUM MERGE ◆ V1(17:5) + FIX-A/B/C/E/G/H ◆ ثقة≥60 · 85%→×2 ◆</div>
  </div>
  <div id="cbLogFloat">
    <div id="cbLogHdr">
  ...
  function initUI() {
    ...
    if (titleEl) {
      ...
      titleEl.textContent = '⚡ QUANTUM MERGE' + (badges.length ? ' ' + badges.join('') : '');
    }
...
;
    ...
    _updateKellyDisplay();
    _updatePPTDisplay();
    addLog('🚀 QUANTUM MERGE V13.4 — أساس V1 الرابح + حُرّاس V2 الدفاعية | مضاعفة ×2 @ثقة≥85', 'signal');
    updateStatsUI();
