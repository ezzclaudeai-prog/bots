// ==UserScript==
// @name         ⚡ V21_MARKETSTATE — Dynamic Frame + Per-Pair Market State + Risk Projection
// @namespace    candle-pro-strategy-v21-marketstate
// @version      21.0.0
// @description  V21 — كشف الفريم الديناميكي (changeSymbol.period) + محرك حالة السوق لكل زوج + إسقاط المخاطر قبل الدخول
// @author       aoirusra
...
(function (W) {
  ...
  const CFG = {
    ...
    TICKPULSE_ENABLED       : true,
    TICKPULSE_MS            : 100,         // فحص كل 100ms
    TICKPULSE_WIN_MS        : 1500,        // نافذة قياس الميل
    TICKPULSE_MIN_TICKS     : 3,           // حد أدنى تيكات
    TICKPULSE_MIN_REL       : 0.000060,   // ✅ [FIX-LOSS] رُفع من 30e-6 إلى 60e-6 — يحذف ميول الضجيج الضعيفة (37-57e-6)
    TICKPULSE_COOLDOWN_MS   : 4000,        // ✅ [FIX-LOSS] رُفع من 1000 إلى 4000 — لا pulse متكرر بعد 4 ثواني
    TICKPULSE_BASE_CONF     : 83,          // ✅ [FIX-LOSS] رُفع من 80 إلى 83 — يتوافق مع الميل الأدنى الجديد
    POST_RESULT_COOLDOWN_MS : 3000,        // ✅ [FIX-LOSS] جديد — انتظر 3 ثواني بعد أي نتيجة (فوز أو خسارة)
    ...
    // ─── [V21] كشف الفريم الديناميكي (changeSymbol.period) ────────────────────
    //   التجسس أكّد: changeSymbol يحمل period بالثواني الفعلية (5،10،15،30،60،120…)
    //   نقرؤه ونحدّث الفريم فوراً عند تبديل المستخدم — بدل التقاطه مرة واحدة فقط.
    DYNAMIC_FRAME_ENABLED   : true,
    // ─── [V21] محرك حالة السوق لكل زوج (MarketState) ──────────────────────────
    MARKETSTATE_ENABLED     : true,        // regime ∈ {QUIET, TRENDING_UP, TRENDING_DOWN, CHOPPY, NEUTRAL}
    MARKETSTATE_MIN_CANDLES : 8,           // أقل عدد شموع لتقييم الحالة
    MARKETSTATE_UPDATE_MS   : 500,         // لا تُعِد التقييم أكثر من مرة كل 500ms
    MARKETSTATE_EFF_TREND   : 0.45,        // كفاءة كاوفمان ≥ هذه + ميل واضح = ترند
    MARKETSTATE_EFF_CHOPPY  : 0.22,        // كفاءة < هذه = متذبذب (السعر يتحرك ولا يصل)
    MARKETSTATE_QUIET_FRAC  : 0.60,        // تقلّب آخر 3 شموع < هذا × التقلّب الأساسي = هادئ
    // ─── [V21] إسقاط المخاطر — احسب الإغلاق المتوقع وهامش الخسارة قبل الدخول ──
    //   طلب المستخدم: «الكود يعرف سعر الدخول، يحسب سعر الإغلاق، يضع هامش حد الخسارة،
    //   ثم يدخل في الوقت المناسب بدقة». لأنها خيارات ثنائية (لا خروج مبكر)، نطبّقها
    //   كبوابة ما‑قبل‑الدخول: لا ندخل إلا إذا تجاوز الإغلاق المتوقع ضجيج التيك (الهامش).
    RISKPROJ_ENABLED        : true,
    RISKPROJ_VEL_MS         : 1500,        // نافذة قياس السرعة (عائد/ثانية) لإسقاط الإغلاق
  ...
  function _snapTradeDuration(secs) {
    ...
    if (CFG.SCALP_FIXED_ENABLED) {
      ...
      fx = Math.max(HARD_MIN, Math.min(HARD_MAX, fx));
      return snapToPOTime(fx);
    }
    const s = Math.round(Number(secs) || 0);
    // ✅ [HARD-BLOCK] فرض الحدود بشكل صارم: 10 ≤ duration ≤ 60
    return Math.max(HARD_MIN, Math.min(HARD_MAX, s));
  }
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // [V21] محرك حالة السوق لكل زوج (MarketState) — مصدر واحد للحقيقة
  //   يقرأ من الشموع المغلقة (candleBuffers) + ميل التيك (OracleLab) ويصنّف:
  //   regime ∈ { QUIET, TRENDING_UP, TRENDING_DOWN, CHOPPY, NEUTRAL }
  //   - كفاءة كاوفمان منخفضة            → CHOPPY (حركة بلا اتجاه)
  //   - كفاءة عالية + ميل واضح          → TRENDING_UP / TRENDING_DOWN
  //   - تقلّب منكمش + ميل ≈ 0            → QUIET
  //   - غير ذلك                          → NEUTRAL
  // ═══════════════════════════════════════════════════════════════════════
  const marketState = {};   // asset -> { regime, trend, efficiency, atrRel, slopeRel, updatedTs }
  const MarketState = (function () {
    const _lastUpd = {};
    // كفاءة كاوفمان: |صافي التغيّر| / Σ|التغيّرات المتتابعة| (1=ترند نقي، 0=تذبذب)
    function _efficiency(closes) {
    ...
    function update(a) {
      if (!CFG.MARKETSTATE_ENABLED || !a) return null;
      const now = Date.now();
      if (_lastUpd[a] && now - _lastUpd[a] < CFG.MARKETSTATE_UPDATE_MS) return marketState[a] || null;
      _lastUpd[a] = now;
      const buf = candleBuffers[a];
      if (!buf || buf.length < CFG.MARKETSTATE_MIN_CANDLES) return marketState[a] || null;
      const win = buf.slice(-(CFG.TREND_WINDOW_CANDLES || 24));
      const closes = win.map(c => c.close);
      const last = closes[closes.length - 1] || 1;
      // الميل المطبّع (فرق متوسط النصف الأحدث عن الأقدم ÷ السعر)
      const half = Math.floor(closes.length / 2) || 1;
      const avgOld = closes.slice(0, half).reduce((s, p) => s + p, 0) / half;
      const avgNew = closes.slice(-half).reduce((s, p) => s + p, 0) / half;
      const slopeRel = (avgNew - avgOld) / last;
      // التقلّب: متوسط مدى الشمعة (نسبي) — أساسي vs آخر 3 شموع (لكشف الانكماش=QUIET)
      let atrSum = 0; for (const c of win) atrSum += (c.high - c.low);
      const atrBaseRel = (atrSum / win.length) / last;
      const recent = win.slice(-3);
      let atrRecSum = 0; for (const c of recent) atrRecSum += (c.high - c.low);
      const atrRecRel = (atrRecSum / recent.length) / last;
      const eff = _efficiency(closes);
      // التصنيف
      const trendDir = slopeRel >= 0 ? 'UP' : 'DOWN';
      let regime;
      if (eff < CFG.MARKETSTATE_EFF_CHOPPY) {
        regime = 'CHOPPY';
      } else if (eff >= CFG.MARKETSTATE_EFF_TREND && Math.abs(slopeRel) > atrBaseRel * 0.15) {
        regime = trendDir === 'UP' ? 'TRENDING_UP' : 'TRENDING_DOWN';
      } else if (atrRecRel < atrBaseRel * CFG.MARKETSTATE_QUIET_FRAC) {
        regime = 'QUIET';
      } else {
        regime = 'NEUTRAL';
      }
      const prev = marketState[a];
      const st = {
        regime,
        trend: regime === 'TRENDING_UP' ? 'UP' : (regime === 'TRENDING_DOWN' ? 'DOWN' : 'NEUTRAL'),
        efficiency: eff, atrRel: atrBaseRel, slopeRel, updatedTs: now,
      };
      marketState[a] = st;
      if (!prev || prev.regime !== regime) {
        try {
          addLog('🧭 [MARKET] ' + a + ' → ' + _regimeLabel(regime) +
                 ' | كفاءة ' + (eff * 100).toFixed(0) + '% | تقلّب ' + (atrBaseRel * 1e6).toFixed(0) + 'e-6', 'info');
        } catch (_) {}
        if (a === activeAsset) _renderMarketHUD(st);
      } else if (a === activeAsset) {
        _renderMarketHUD(st);
      }
      return st;
    }
    function _regimeLabel(r) {
      return ({
        QUIET: 'هادئ ⏸', TRENDING_UP: 'صاعد ↑', TRENDING_DOWN: 'هابط ↓',
        CHOPPY: 'متذبذب 〰', NEUTRAL: 'محايد ↔',
      })[r] || r;
    }
    function _renderMarketHUD(st) {
      try {
        const el = W.document.getElementById('cbMarketState');
        const effEl = W.document.getElementById('cbMarketEff');
        if (el) {
          el.textContent = _regimeLabel(st.regime);
          const colors = { QUIET: '#8aa0b4', TRENDING_UP: '#00d264', TRENDING_DOWN: '#ff3755', CHOPPY: '#ffb020', NEUTRAL: '#999' };
          el.style.color = colors[st.regime] || '#999';
        }
        if (effEl) effEl.textContent = 'eff ' + (st.efficiency * 100).toFixed(0) + '%';
      } catch (_) {}
    }
    function get(a) { return marketState[a] || null; }
    return { update, get, regimeLabel: _regimeLabel };
  })();

  // ═══════════════════════════════════════════════════════════════════════
  // [V21] إسقاط المخاطر (RiskProjector) — يحسب سعر الإغلاق المتوقع وهامش الخسارة
  //   entry  = آخر تيك
  //   projClose = entry × (1 + velocity/ث × مدة_الصفقة × تخميد)
  //   margin = انحراف ضجيج التيك المعياري × MULT
  //   ok     = (الحركة في صالح الاتجاه) ≥ margin  →  الدخول مبرّر حسابياً
  // ═══════════════════════════════════════════════════════════════════════
  function _tickNoiseRel(tb) {
    const n = Math.min(20, tb.length - 1);
    if (n < 3) return 0;
    const rets = [];
    for (let i = tb.length - n; i < tb.length; i++) {
      if (i > 0 && tb[i - 1] > 0) rets.push((tb[i] - tb[i - 1]) / tb[i - 1]);
    }
  ...
  function _projectClose(a, dir, durSec) {
    const tb = tickBuffers[a];
    if (!tb || tb.length < 5) return { ok: true, reason: 'no-data' };   // fail-open
    const entry = tb[tb.length - 1];
    if (!(entry > 0)) return { ok: true, reason: 'no-price' };
    const sv = OracleLab.microSlope(a, CFG.RISKPROJ_VEL_MS);
    const velPerSec = (sv && sv.dt > 0) ? (sv.rel / (sv.dt / 1000)) : 0;   // عائد نسبي/ثانية
    const projMoveRel = velPerSec * durSec * CFG.RISKPROJ_DAMP;
    const projClose = entry * (1 + projMoveRel);
    const noiseRel = _tickNoiseRel(tb);
    const margin = noiseRel * CFG.RISKPROJ_MARGIN_MULT;
    const favor = (dir === 'BUY') ? projMoveRel : -projMoveRel;            // الحركة في صالح الصفقة
    const ok = favor >= margin;
    return { ok, entry, projClose, projMoveRel, margin, favor, noiseRel };
  }
  ...
  function recordTrade(win, wasTVE) {
    ...
    tradeExec = false;
    _lastResultTs = Date.now();  // ✅ [FIX-LOSS] سجّل وقت النتيجة — يمنع pulse فوري بعد الإغلاق
      ...
      if (typeof data === 'string' && data.startsWith('42')) {
        try {
          ...
          if (Array.isArray(arr)) {
            ...
            if (evName==='changeSymbol' && payload?.asset) {
              onActiveAsset(String(payload.asset), 'changeSymbol_send');
              // ✅ [V21] الفريم الديناميكي — changeSymbol يحمل period بالثواني الفعلية
              if (CFG.DYNAMIC_FRAME_ENABLED && Number.isFinite(+payload.period)) onPlatformTimeframe(+payload.period, 'changeSymbol');
            }
  ...
  function handleTextMessage(raw, wsRef) {
    ...
    if (evName==='changeSymbol' && data?.asset) {
      onActiveAsset(data.asset, 'changeSymbol');
      // ✅ [V21] الفريم الديناميكي — اقرأ period من changeSymbol المُستقبَل أيضاً
      if (CFG.DYNAMIC_FRAME_ENABLED && Number.isFinite(+data.period)) onPlatformTimeframe(+data.period, 'changeSymbol');
    }
  ...
  function processHistoryFast(asset, period, history) {
    ...
    // ✅ [V21] الفريم من التاريخ مصدر احتياطي فقط: يملأ الفراغ ولا يطغى على changeSymbol خلال القفل
    if (!candlePeriod && period && period>0 && Date.now() > _periodLockUntil) onPlatformTimeframe(period, 'history');
    ...
    if (newCandles.length > 0) {
      ...
      try { MarketState.update(a); } catch(_) {}   // ✅ [V21] حدّث حالة السوق من التاريخ
  ...
  function onPlatformTimeframe(secs, source) {
    if (!Number.isFinite(secs) || secs<1 || secs>86400) return;   // ✅ [V21] حتى 1د/15د/يومي
    if (secs === candlePeriod) return;
    const prev = candlePeriod;
    candlePeriod = secs; durSource = source||'platform'; _lastDetectedPeriod = secs; _lastDetectedCount = 999;
    _periodLockUntil = Date.now() + 30000;
    // ✅ [V21] الفريم تغيّر فعلاً → امسح شموع الزوج النشط كي تُعاد بناؤها على الفريم الجديد
    //   (وإلا تختلط شموع 5ث القديمة مع شموع 60ث الجديدة ويفسد كشف الاتجاه/الحالة)
    if (prev && prev !== secs && activeAsset) {
      delete candleBuffers[activeAsset];
      delete currentCandles[activeAsset];
    }
    try {
  ...
  function onTick(asset, price, serverTs, srcRole) {
    ...
    try { if (a === activeAsset) MarketState.update(a); } catch(_) {}  // ✅ [V21] حالة السوق (مُخمّدة داخلياً كل 500ms)
    totalTicks++;
    ...
    if (st.prev !== null && seconds > st.prev) {
      ...
      if (cc && cc.prices && cc.prices.length >= 2) {
        ...
        if (candle) {
          ...
          try { MarketState.update(a); } catch(_) {}   // ✅ [V21] حدّث حالة السوق عند إغلاق الشمعة
        }
      }
    }
  ...
  /* ══════════════════════════════════════════════════════════════
     LIQUID GLASS TRADING ORB — ChatGPT Voice Orb × Apple Vision
     Ultra-premium floating widget for PocketOption signals
     ══════════════════════════════════════════════════════════════ */
  ...
  <div id="cbPanel">
    ...
    <div id="cbScrollArea">
      ...
      <div class="cb-ind-row" title="[V21] حالة السوق لكل زوج — هادئ/صاعد/هابط/متذبذب">
        <span class="cb-ind-lbl">حالة السوق</span>
        <span class="cb-ind-val" id="cbMarketState">–</span>
        <span class="cb-ind-badge" id="cbMarketEff" style="font-size:8px">eff –</span>
      </div>
      <div class="cb-ind-row">
  ...
  // ══════════════════════════════════════════════════════════════════════
  // § LIQUID GLASS TRADING ORB — showSignalPopup / hideSignalPopup
  //   ✅ يستبدل cbSignalPopup القديم بـ Orb متحرك بالكامل
  //   ✅ Draggable — Real-time — GPU-only animations — Ultra-low CPU
  // ══════════════════════════════════════════════════════════════════════

  // ─── مؤقتات الأوراكل ───
  // أسماء الأنماط بالعربية
  const _patNames = {
    'interval_signal':'إشارة فاصل','uhnf_arb':'⚡ تحكيم WSS','tick_pulse':'نبضة تيك',
    '3bullish':'3 شموع صاعدة','3bearish':'3 شموع هابطة','bullish_engulfing':'ابتلاع صاعد',
    'bearish_engulfing':'ابتلاع هابط','doji_reversal':'انعكاس دوچي','momentum_continuation':'استمرار زخم'
  };
  ...
  function _orbSetTimerRing(pct, state) {
    ...
    if (!el) return;
    const colors = { buy:'rgba(70,217,142,0.55)', sell:'rgba(220,38,38,0.55)', idle:'rgba(0,200,255,0.4)' };
  ...
  // ─── تحديث حالة الـ Orb (buy/sell/idle) ───
  ...
  function showSignalPopup(opts) {
    ...
    if (_spActive) {
      addLog('⏳ [ORB] إشعار نشط — تجاهل الإشارة الجديدة (لا طابور)', 'info');
      return;
    }
    ...
    if (arrowEl) { arrowEl.textContent = isBuy ? '▲' : '▼'; arrowEl.style.color = isBuy ? '#46d98e' : '#ff4d4d'; }
    const dirEl = W.document.getElementById('cbOrbDir');
    ...
    // العمر والعد التنازلي (يُحدَّثان بالمؤقت)
    ...
    // احتمال الفوز = الثقة (قابل للتوسع)
    const prob    = opts.confidence || 0;
    const probBar = W.document.getElementById('cbOrbProbBar');
    ...
    _spCountdownTimer = _v11_setInterval(() => {
      ...
      if (countEl) {
        ...
        } else { countEl.textContent = remainSec + ' ث'; }
      }
      // عمر الصفقة
      const ageSec = Math.floor(elapsed / 1000);
      if (ageEl) ageEl.textContent = ageSec >= 60 ? Math.floor(ageSec/60) + ':' + String(ageSec%60).padStart(2,'0') : ageSec + ' ث';
      // حلقة التايمر
      _orbSetTimerRing(pct, state);
      // السعر الحالي (من tickBuffer)
      const tb = tickBuffers && tickBuffers[activeAsset];
      if (curPriceEl && tb && tb.length) {
      ...
      if (remain <= 0 && _spCountdownTimer) { clearInterval(_spCountdownTimer); _spCountdownTimer = null; }
    }, 250);
    ...
    _spAutoCloseTimer = setTimeout(() => {
      hideSignalPopup();
      if (_spSignalQueue) { _spSignalQueue = null; }
    }, autoCloseMs);

    addLog('🟢 [ORB] ' + (isBuy ? '▲ شراء' : '▼ بيع') + ' | ' + assetName + ' @ ' +
           (opts.price || 0).toFixed(5) + ' | ' + durSec + 'ث | ثقة ' + prob + '%', 'signal');
  }
  ...
  function _initSignalPopup() {
    ...
    if (closeX) closeX.addEventListener('click', (e) => { e.stopPropagation(); hideSignalPopup(); });
    ...
    orb.addEventListener('pointerdown', (e) => {
      ...
      _dragging = true; _ox = orb.offsetLeft; _oy = orb.offsetTop;
...
;
  ...
  const DualWSSManager = (() => {
    ...
    let _lastResultTs = 0;  // ✅ [FIX-LOSS] وقت آخر نتيجة صفقة (فوز أو خسارة)
    function tickPulse(asset) {
      ...
      // ✅ [FIX-LOSS] انتظر POST_RESULT_COOLDOWN_MS بعد أي نتيجة
      //   يمنع "يُطلق 300ms بعد الفوز/الخسارة" — الخلل الرئيسي في السجل
      if (_lastResultTs > 0 && now - _lastResultTs < CFG.POST_RESULT_COOLDOWN_MS) return;
      _lastPulseCheck = now;
      ...
      // ✅ [FIX-LOSS] تحقق إضافي: الميل الأطول (3000ms) يجب ألا يكون معاكساً تماماً
      //   يمنع الدخول ضد الترند الكبير (سبب خسارات BUY في سوق هابط)
      const sLong = OracleLab.microSlope(a, 3000);
      if (sLong && Math.abs(sLong.rel) > CFG.TICKPULSE_MIN_REL * 3) {
        const longDir = sLong.rel > 0 ? 'BUY' : 'SELL';
        if (longDir !== dir) return; // ✅ لا تدخل عكس الترند الكبير
      }
      ...
      // ═══ [V21] بوابة إسقاط المخاطر — لا ندخل إلا إذا الإغلاق المتوقع مربح بهامش ═══
      //   تطبيق طلب المستخدم: «اعرف سعر الدخول، احسب سعر الإغلاق، ضع هامش حد الخسارة،
      //   ثم ادخل في الوقت المناسب بدقة». نحسب مدة الصفقة أولاً لإسقاط الإغلاق عليها.
      if (CFG.RISKPROJ_ENABLED) {
        const _projDur = _snapTradeDuration(_computeSignalDuration(conf, 10, 'tick_pulse'));
        const ms = MarketState.get(a);
        if (CFG.RISKPROJ_BLOCK_CHOPPY && ms && ms.regime === 'CHOPPY') {
        ...
        if (!pj.ok) {
          addLog('🚫 [RISK] إسقاط ' + dir + ' غير مربح: حركة متوقعة ' + (pj.projMoveRel * 1e6).toFixed(1) +
                 'e-6 < هامش ' + (pj.margin * 1e6).toFixed(1) + 'e-6 (' + _projDur + 'ث) — تخطّي', 'error');
          return;
        }
        if (pj.entry) {
          addLog('🎯 [RISK] ' + dir + ' @ ' + pj.entry.toFixed(5) + ' → إغلاق متوقع ' + pj.projClose.toFixed(5) +
                 ' | ربح ' + (pj.favor * 1e6).toFixed(1) + 'e-6 ≥ هامش ' + (pj.margin * 1e6).toFixed(1) + 'e-6 ✓', 'signal');
        }
      }
    ...
    function _enqueueSignal(signal) {
      ...
      // ✅ [FIX-DEDUP] رفض إشارة متكررة في نفس الاتجاه خلال 2 ثانية
      if (_lastSignal && _lastSignal.direction === signal.direction &&
          Date.now() - _lastSignal.timestamp < 2000) return;

      // ✅ [FIX-DEDUP] رفض إشارة معاكسة قبل انتهاء مدة الإشارة السابقة
      //   لا تعرض SELL بعد BUY (أو العكس) مباشرة — انتظر انتهاء المدة الكاملة
      if (_lastSignal && _lastSignal.direction !== signal.direction) {
        const prevDurSec = _computeSignalDuration(_lastSignal.confidence, 15, _lastSignal.pattern);
        const prevExpiry = _lastSignal.timestamp + (prevDurSec * 1000) + 3000; // مدة + 3ث هامش
        if (Date.now() < prevExpiry) {
          addLog('🚫 [DEDUP] رفض ' + signal.direction + ' معاكسة — السابقة (' + _lastSignal.direction + ') لم تنتهِ بعد (تبقّى ' + Math.round((prevExpiry - Date.now()) / 1000) + 'ث)', 'info');
          return;
        }
      }
